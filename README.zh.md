...
`validate` 会在用户提交时同步执行。返回 `true` 表示接受当前值，返回 `false` 会显示通用错误，返回字符串则会把该字符串作为错误显示在当前输入框下方。当输入输出流都是交互式终端时，带校验的输入会从新行开始。校验失败时会保留原输入并原位重绘，不会产生重复的输入行。修改时若文本超过终端高度，只显示可编辑的尾部，返回值仍保留完整内容。如果输入输出流不是交互式终端，校验失败会抛出 `Error`，因为此时无法让用户原位修改。
